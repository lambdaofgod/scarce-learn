...
def load_split(split_image_filenames):
    image_paths = glob.glob(os.path.join(CUB_DATA_PATH, 'images', '*/**'), recursive=True)
...
def load_cub(image_size=(128, 128)):
    train_image_paths = load_split('train.txt')
    test_image_paths = load_split('test.txt')
