...
def _get_image_basepaths(split_text_filename):
    image_paths = open(
        os.path.join(CUB_IMAGE_DATA_PATH, "lists", split_text_filename)
    ).readlines()
    return [os.path.basename(path[:-1]) for path in image_paths]


def _load_images_with_labels(image_paths, image_size):
    images = np.stack([imresize(imread(path), image_size) for path in image_paths])
    labels = np.array([path.split("/")[-2] for path in image_paths])
...
def load_split(images_dir, split_image_filenames):
    ...
    return [
        ...
        if os.path.basename(image_path) in split_image_paths
    ]
...
def prepare_data(data_path):
    ...
    expected_directories = [
        os.path.join(data_path, path) for path in ["images", "lists", "attributes"]
    ]
...
def download_tars(data_path):
    ...
    expected_tars = [os.path.join(data_path, subset + ".tgz") for subset in subsets]
    template_link = "http://www.vision.caltech.edu/visipedia-data/CUB-200/{}.tgz"
    links = [template_link.format(subset) for subset in subsets]
